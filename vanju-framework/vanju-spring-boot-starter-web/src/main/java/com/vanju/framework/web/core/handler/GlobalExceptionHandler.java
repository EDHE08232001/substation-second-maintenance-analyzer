package com.vanju.framework.web.core.handler;
...
import com.vanju.framework.common.exception.util.ServiceExceptionUtil;
import com.vanju.framework.common.pojo.CommonResult;
import com.vanju.framework.common.util.collection.SetUtils;
...
import com.vanju.module.infra.api.logger.ApiErrorLogApi;
import com.vanju.module.infra.api.logger.dto.ApiErrorLogCreateReqDTO;
...
import static com.vanju.framework.common.exception.enums.GlobalErrorCodeConstants.*;
...
//        errorLog.setTraceId(TracerUtils.getTraceId());
        ...
        Map<String, Object> requestParams = MapUtil.<String, Object>builder().put("query", JakartaServletUtil.getParamMap(request)).put("body", JakartaServletUtil.getBody(request)).build();
    ...
    private CommonResult<?> handleTableNotExists(Throwable ex) {
        ...
        if (message.contains("pay_")) {
            log.error("支付模块-表结构未导入");
        }
        // 8. AI 大模型
        if (message.contains("ai_")) {
            log.error("AI 大模型 -表结构未导入");
        }
        // 9. IOT 物联网
        if (message.contains("iot_")) {
            log.error("IOT 物联网- 表结构未导入");
        }
        return null;
    }
